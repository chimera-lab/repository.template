...
**Active mappings**: generated from `.chimera-lab/config.json` — run `cmr docs render` to refresh.
...
- Default location: repository `.chimera-lab/config.json` under `llm.headerPrompts`
...
**Registered directives** (generated by CMR — run `cmr docs render` to refresh):
